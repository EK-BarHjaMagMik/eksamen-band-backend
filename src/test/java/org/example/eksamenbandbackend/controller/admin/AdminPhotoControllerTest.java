...
class AdminPhotoControllerTest {
    ...
    @Test
    void batchUpdatePhotos_returnsUpdatedPhotosOnSuccess() throws Exception {
        PhotoResponse r1 = new PhotoResponse(1L, "/uploads/1.jpg", null, null, "Cap1", LocalDate.of(2026, 5, 1), "P1", null);
        PhotoResponse r2 = new PhotoResponse(2L, "/uploads/2.jpg", null, null, "Cap2", LocalDate.of(2026, 5, 2), "P2", null);

        when(photoService.batchUpdatePhotos(any())).thenReturn(List.of(r1, r2));

        mockMvc.perform(patch("/api/admin/photos")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                        "photoIds": [1,2],
                ...
                .andExpect(jsonPath("$[0].url").value("/uploads/1.jpg"))
                .andExpect(jsonPath("$[1].id").value(2));
    }

    @Test
    void batchUpdatePhotos_returnsBadRequestWhenServiceReports() throws Exception {
        when(photoService.batchUpdatePhotos(any()))
                .thenThrow(new ResponseStatusException(HttpStatus.BAD_REQUEST, "One or more photo IDs not found"));
    ...
    @Test
    void batchUpdatePhotos_returnsBadRequestWhenRequestBodyIsInvalid() throws Exception {
        mockMvc.perform(patch("/api/admin/photos")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
                .andExpect(status().isBadRequest());
